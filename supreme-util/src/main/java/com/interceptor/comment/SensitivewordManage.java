...
public class SensitivewordManage {
	 ...
	 * 构造函数，初始化敏感词库
	 */
	public SensitivewordManage() {
		sensitiveWordMap = new LoadSensitiveWord().initKeyWord();
	}
		...
		for (int i = beginIndex; i < txt.length(); i++) {
			...
			if (nowMap != null) { // 存在，则判断是否为最后一个
				...
				if ("1".equals(nowMap.get("isEnd"))) { // 如果为最后一个匹配规则,结束循环，返回匹配标识数
					...
					if (SensitivewordManage.minMatchTYpe == matchType) { // 最小规则，直接返回,最大规则还需继续查找
						break;
					}
				}
			} else { // 不存在，直接返回
	...
	// String string = "太多的伤感情怀也许只局限于饲养基地
