...
public class IompSwaggerConfig {
    /**
     *
     * 配置参数
     *
...
//        aParameters.add(parameter);
        ...
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(buildApiInf()).globalOperationParameters(aParameters)
                .select().apis(RequestHandlerSelectors.basePackage("com.iomp.controller"))// controller路径
    ...
    private ApiInfo buildApiInf() {
        return new ApiInfoBuilder().title("综合业务平台API").termsOfServiceUrl("").description("综合业务平台接口")
                .contact(new Contact("综合业务平台", "", "zhouxingyu@chtwm.com")).build();
