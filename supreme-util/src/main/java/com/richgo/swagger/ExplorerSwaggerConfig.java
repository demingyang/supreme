...
public class ExplorerSwaggerConfig {
	...
	public Docket buildDocket() {
		...
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(buildApiInf()).globalOperationParameters(aParameters)
				.select().apis(RequestHandlerSelectors.basePackage("com.explorer.controller"))// controller路径
	...
	private ApiInfo buildApiInf() {
		return new ApiInfoBuilder().title("资管量API").termsOfServiceUrl("").description("资管量接口")
				.contact(new Contact("资管量", "", "zhouxingyu@chtwm.com")).build();
